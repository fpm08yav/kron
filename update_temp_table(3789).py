...
from common_functions import (
    ...
    execute, fetchall, execute_file, levels_table, create_chains,
    add_breaket, get_max_level, get_difference_days_list, get_list_hour,
    get_drop_partition_sql, put_headers, update_ab_table,
...
BEGIN_HOUR = 1472688000     # unix_timestamp('2016-09-01 00:00:00')
NUM_WORKERS = 2
TABLE_WIN_FAIL = 'win_fail_3789_pq'
TABLE_BOOSTS = 'boosts_3789_pq'
...
def get_insert_boosts_sql(hour, app):
            ...
            (
                select
                    ...
                    sum(nvl(cast(regexp_extract(
                        p.payload,
                        concat(boosts.name, '\\":\\"(\\\\d+)'), 1
                    ) as int), 0)) as boost_count
                from
                    payload p,
                    --all_events_3789_pq b,
                ...
                where
                    ...
                    /*and c.free_coins_balance + c.real_coins_balance < 100000*/
                    ...
                    and c.cheater != 1
                    and (
                        (c.valid_payer = 0 and c.real_coins_balance = 0) or
                        c.valid_payer = 1)
                group by
                ...
                (
                    select
                        ...
                        sum(
                            if(nvl(cast(regexp_extract(
                                p.payload,
                                'AdditionalMoves\\":\\"(\\\\d+)',
                                1
                            ) as int), 0) >= cast(boost_number.num as int),1,0)
                        ) as boost_count
                    from
                        payload p,
                    ...
                    select
                        ...
                        sum(
                            if(nvl(cast(regexp_extract(
                                p.payload,
                                'AdditionalTime\\":\\"(\\\\d+)',
                            1) as int), 0) >= cast(boost_number.num as int),1,0
                        )) as boost_count
                    from
                        payload p,
                        (%(boost_number)s) boost_number,
    ...
    """ % {
        ...
        'next_day': next_day,
        'additional':
            'values(("AdditionalMoves" as name), ("AdditionalTime"))',
        'boost_number':
            'values(("1" as num), ("2"), ("3"), ("4"), ("5"), ("6"), ("7"))',
        # 'ab_table': ab_table
    }
    return sql
    ...
    sql = """
        ...
        (
            select
                ...
                (
                    ...
                    sum(nvl(p.am_count, 0))) as all_boosts
            from
                (
                    select
                        p.event_id,
        ...
        from(
            select
                ...
                sum(
                    decode(p.event_name, 1, decode(p.all_boosts, 0, 1, 0), 0)
                ) as win_without_boosts,
            ...
            from
                ...
                (
                    ...
                    (
                        ...
                        group by
                            year, month, day, app_version, level_version,
                            ab_group, w.device_region, w.payer,
                            chain,
                            w.level
                     ) s
                ) w
    ...
    while attempt < NUMBER_ATTEMPT and not complete:
        try:
            ...
            try:
                ...
                pass
            sql = get_insert_boosts_sql(hour, '3789')
            # logging.info(sql)
            execute(sql)
    ...
    while attempt < NUMBER_ATTEMPT and not complete:
        try:
            ...
            try:
                ...
                pass
            sql = get_insert_win_fail_sql(hour, '3789')
            # logging.info(sql)
            execute(sql)
        except socket.timeout, impala.error.DisconnectedError:
            attempt += 1
