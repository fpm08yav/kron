...
LOCK_FILE = 'conversion.lock'
LOGGING_PATH = '/home/yansen-a/analytics/logs'

NUMBER_ATTEMPT = 10
TIME_SLEEP = 10 * 60

APPS = ['3606', '3791', '3928', '3789', '3858', '3790', '3426', '3444']
# APPS = ['3789']
...
CUBE_APP = {
    '3444': 'cube_conv_fd',
    '3426': 'cube_conv_fd_an',
    '3606': 'cube_conv_fd_am',
    '3789': 'cube_conv_gs',
    '3790': 'cube_conv_gs_an',
    '3791': 'cube_conv_gs_am',
    '3858': 'cube_conv_gs_fb',
    '3928': 'cube_conv_zm'
}
...
if __name__ == '__main__':
    if not try_lock(LOCK_FILE):
        ...
        for app in APPS:
            ...
            path_file = '/home/yansen-a/analytics/local_cubes/temp/%s' % CUBE_APP[app]
            # file_name = 'conversion_otval%s' % app
            date_format = '%Y-%m-%d'
            ...
            if seg_players != seg_users:
                ...
                last_day = datetime.strptime(
                    get_last_day('local_cubes/logs/conversion_otval_%s.txt' % app), date_format)
                if last_day:
                ...
                if end > last_day:
                    ...
                    while start <= end:
                        file_name = datetime.strftime(start, '%Y_%m_%d')
                       ...
                       # ab_table = ids_table(get_ab_values(app,start))
                        ...
                        res = fetchall(sql)
                        with open(
                            os.path.join(path_file, file_name + '.csv'), 'a'
                        ) as f:
                            for row in res:
                                f.write('%s\n' % ';'.join(row))
                        start += timedelta(days=1)
                        i += 1
                        logger.info((CUBE_APP[app], file_name))
                        relocation_file(CUBE_APP[app], file_name)
                    push_last_day(
                        'local_cubes/logs/conversion_otval_%s.txt' % app,
