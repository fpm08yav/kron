...
from common_functions import (
    ...
    execute, execute_file, levels_table, fetchall, add_breaket,#, create_chains,
    get_max_level, get_difference_days_list, get_list_hour,
...
NUM_WORKERS = 3
TABLE_WIN_FAIL = 'win_fail_3791_pq'
TABLE_BOOSTS = 'boosts_3791_pq'
    ...
    sql = """
        ...
        (
            select
                ...
                payload
            from
                all_events_3791_pq p left join ab
            on
...
def update_boost(hour):
    ...
    while attempt < NUMBER_ATTEMPT and not complete:
        try:
            ...
            #     pass
            sql = get_insert_boosts_sql(hour, '3791')
            # logging.info(sql)
            execute(sql)
...
def ubdate_win_fail(hour):
    ...
    while attempt < NUMBER_ATTEMPT and not complete:
        try:
            ...
            # except impala.error.RPCError:
            #     pass
            sql = get_insert_win_fail_sql(hour, '3791')
            # logging.info(sql)
            execute(sql)
        except socket.timeout, impala.error.DisconnectedError:
            attempt += 1
...
def create_csv_files(first_hour, last_hour, headers, max_level, max_chain):
    ...
    cube_name = 'cube_difficult_var_gs_am'
    last_hour_str = (
