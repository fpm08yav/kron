...
from common_functions import (
    ...
    execute, execute_file, levels_table, fetchall,  #create_chains,
    get_max_level, get_difference_days_list, get_list_hour, add_breaket,
...
NUM_WORKERS = 1
TABLE_WIN_FAIL = 'win_fail_3790_pq'
TABLE_BOOSTS = 'boosts_3790_pq'
    ...
    sql = """
        ...
        (
            select
                ...
                payload
            from
                all_events_3790_pq p left join ab
            on
    ...
    sql = """
        ...
        (
            select
                ...
                decode(
                    LENGTH(nvl(w.device_region, '')), 2, w.device_region, '--')  as device_region,
                    -- LENGTH(nvl(w.device_region, 0)), 2, w.device_region, '--')  as device_region,
    ...
    sql = """
        ...
        (
            ...
            where
                ...
                and nvl(b.device_region, '') = nvl(w.device_region, '')
            group by
    ...
    while attempt < NUMBER_ATTEMPT and not complete:
        try:
            ...
            try:
                ...
                pass
            sql = get_insert_boosts_sql(hour, '3790')
            # logging.info(sql)
            execute(sql)
    ...
    while attempt < NUMBER_ATTEMPT and not complete:
        try:
            ...
            try:
                ...
                pass
            sql = get_insert_win_fail_sql(hour, '3790')
            # logging.info(sql)
            execute(sql)
        except socket.timeout, impala.error.DisconnectedError:
            attempt += 1
...
def create_csv_files(first_hour, last_hour, headers, max_level, max_chain):
    ...
    cube_name = 'cube_difficult_var_gs_an'
    last_hour_str = (
