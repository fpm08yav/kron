...
from common_functions import (
    try_lock, delete_lock, StreamToLogger, get_full_list_hour,
...
TABLE_WIN_FAIL = 'win_fail_3606_pq_test_s'
TABLE_BOOSTS = 'boosts_3606_pq_test_s'
...
def get_new_ab_groups_sql():
    ...
    if ab_groups:
        ...
        """ % {
            ...
            'seg_hour': get_full_list_hour('seg_users_3606_pq')[-1]
        }
    else:
        return """
             SELECT
...
def get_create_table_sql(name):
    return """
        CREATE TABLE IF NOT EXISTS %s_fd_am
        (
          id smallint,
          name varchar(255),
    ...
    sql = """
        ...
        (
            select
                ...
                payload
            from
                all_events_3606_pq p left join ab
            on
...
def get_difference_days(first_table, second_table, first_hour=None):
    ...
    difference_days = map(
        lambda hour: int(hour[0]),
        filter(
            lambda hour: hour[0].isdigit() and int(hour[0]) >= last_hour, partitions
        )
    )
...
def get_day_list(cursor, table):
    ...
    partitions = fetchall('show partitions seg_players_3606_pq;')
    day_list = map(
        lambda hour: int(hour[0]),
        filter(
...
def create_csv_files(first_hour, last_hour, headers, max_level, max_chain):
    cube_name = 'cube_difficult_var_fd_am'
