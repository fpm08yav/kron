...
LOCK_FILE = 'update_temp_table1.lock'
...
def get_insert_boosts_sql(hour):
    ...
    sql = """
        insert into %(table)s (
            app_version, level_version, level, ab_group, device_region, payer, chain, boost, boost_count
        )
        ...
        WITH ab AS (
            ...
            FROM
                ...
                FROM
                    ab_group_3444_pq
                WHERE
                    hour = unix_timestamp(days_add(to_date(from_unixtime(%(hour)s)), 1))
                ) AS ids
            WHERE
        ...
        select
            app_version, level_version, level_number, cast(ab_group as VARCHAR(255)), cast(device_region as VARCHAR(255)), payer, chain_number, cast(boost as VARCHAR(255)), boost_count
        from
            (
                select
                    ...
                    sum(nvl(cast(regexp_extract(b.payload, concat(boosts.name, '\\":\\"(\\\\d+)'), 1) as int), 0)) as boost_count
                from
                    payload p,
                    all_events_3444_pq b,
                ...
                where
                    ...
                    and b.hour = %(hour)s
                    and b.event_type = 'event'

                    and c.hour = unix_timestamp(days_add(to_date(from_unixtime(%(hour)s)), 1))
                ...
                (
                    select
                        ...
                        sum(if(nvl(cast(regexp_extract(b.payload, 'AdditionalMoves\\":\\"(\\\\d+)', 1) as int), 0) >= cast(boost_number.num as int),1,0)) as boost_count
                    from
                        payload p,
                ...
                (
                    select
                        ...
                        sum(if(nvl(cast(regexp_extract(b.payload, 'AdditionalTime\\":\\"(\\\\d+)', 1) as int), 0) >= cast(boost_number.num as int),1,0)) as boost_count
                    from
                        payload p,
                        (%(boost_number)s) boost_number,
                        all_events_3444_pq b,
        ...
        /*where
            boost_count != 0*/
        order by chain_number, level_number
    """ % {
        'table': TABLE_BOOSTS,
        'hour': hour,
    ...
    sql = """
                        ...
                        from
                            (
                                select
                                    ...
                                    cast(regexp_extract(payload, 'AdditionalMoves\\":\\"(\\\\d+)', 1) as int) as am_count,
                                    nvl(cast(regexp_extract(payload, 'LevelsVersion\\":\\"(\\\\d+)', 1) as int), -1) as level_version,
                                    /*nvl(regexp_extract(payload, "ABGroups\\":\\"(\\\\w+)", 1), '--') as ab_group,*/
                                    nvl(ab.ab_names, '--') as ab_group,
                                    regexp_extract(payload, 'Reason\\":\\"(\\\\w+)', 1) as reason
                                from
                                    all_events_3444_pq p left join ab
        ...
        from(
            ...
            from
                ...
                ON c.event_user = s.event_user
            where
                c.hour = unix_timestamp(days_add(to_date(from_unixtime(%(hour)s)), 1))
...
def get_cube_create_sql(first_hour, last_hour, max_level, max_chain):
    sql = """
        select
            ...
            win_without_boosts_count, decode(ab_group, '', '--', ab_group), total_monetization
        from
        (
            select
...
def get_difference_days(first_table, second_table, first_hour=None, second_hour=None):
    if not first_hour:
    ...
    difference_days = map(
        lambda hour: int(hour[0]),
        filter(
            # lambda hour: hour[0].isdigit() and int(hour[0]) >= last_hour and int(hour[0]) <= t_hour, partitions
...
def create_faik_record(cursor, first_table, second_table, last_hour):
    pass
    # sql = get_drop_table_sql(first_table)
    # cursor.execute(sql)
    # sql = get_drop_table_sql(second_table)
    # cursor.execute(sql)
    # try:
    #     print cursor.execute(sql)
    # except impala.error.RPCError:
    #     pass
    # sql = get_create_faik_record_sql(first_table, last_hour)
    # cursor.execute(sql)
    # sql = get_create_faik_record_sql(second_table, last_hour)
    # cursor.execute(sql)
...
def get_day_list(cursor, table):
    ...
    day_list = map(
        lambda hour: int(hour[0]),
        filter(
            lambda hour: hour[0].isdigit() and int(hour[0]) >= first_hour, partitions
        )
    )
    day_list.sort()
    return day_list
...
if __name__ == '__main__':
    if not try_lock(LOCK_FILE):
        ...
        if min(list_seg_day) != max(list_seg_day):
            ...
            # difference_days_w = [[1468454400, 1468540800]]
            while difference_days_w:
                ...
                for hour1, hour2 in difference_days_w:
                    ...
                    logging.info((day1.strftime('%Y-%m-%d'), day2.strftime('%Y-%m-%d')))
                    # hour_list = get_list_hour('all_events_3444_pq', hour1, hour2)
